...
using Edge = MctsGraphOfConvexSets::Edge;
using EdgeId = MctsGraphOfConvexSets::EdgeId;
using Transcription = MctsGraphOfConvexSets::Transcription;
using Vertex = MctsGraphOfConvexSets::Vertex;
using VertexId = MctsGraphOfConvexSets::VertexId;
...
MathematicalProgramResult SolveMainProgram(
    const MathematicalProgram& prog, const MctsGraphOfConvexSetsOptions& options) {
  ...
  if (options.solver) {
    ...
    try {
      ...
      throw std::runtime_error(
          "MctsGraphOfConvexSets: There is no solver available that can solve "
          "the "
...
MctsGraphOfConvexSets::~MctsGraphOfConvexSets() = default;
...
Binding<Constraint> Vertex::AddConstraint(
    ...
    const std::unordered_set<MctsGraphOfConvexSets::Transcription>&
...
std::unique_ptr<MctsGraphOfConvexSets> MctsGraphOfConvexSets::Clone() const {
  auto clone = std::make_unique<MctsGraphOfConvexSets>();
...
Vertex* MctsGraphOfConvexSets::AddVertex(const ConvexSet& set, std::string name) {
  if (name.empty()) {
...
Vertex* MctsGraphOfConvexSets::AddVertexFromTemplate(const Vertex& v) {
...
Edge* MctsGraphOfConvexSets::AddEdge(Vertex* u, Vertex* v, std::string name) {
...
Edge* MctsGraphOfConvexSets::AddEdgeFromTemplate(Vertex* u, Vertex* v,
                                             const Edge& e) {
  DRAKE_DEMAND(u != nullptr && IsValid(*u));
  DRAKE_DEMAND(v != nullptr && IsValid(*v));
...
const Vertex* MctsGraphOfConvexSets::GetVertexByName(
...
Vertex* MctsGraphOfConvexSets::GetMutableVertexByName(const std::string& name) {
...
const Edge* MctsGraphOfConvexSets::GetEdgeByName(const std::string& name) const {
  for (const auto& [e_id, e] : edges_) {
    if (e->name() == name) {
      return e.get();
    }
...
Edge* MctsGraphOfConvexSets::GetMutableEdgeByName(const std::string& name) {
...
void MctsGraphOfConvexSets::RemoveVertex(Vertex* vertex) {
...
void MctsGraphOfConvexSets::RemoveEdge(Edge* edge) {
...
std::vector<Vertex*> MctsGraphOfConvexSets::Vertices() {
...
std::vector<const Vertex*> MctsGraphOfConvexSets::Vertices() const {
...
bool MctsGraphOfConvexSets::IsValid(const Vertex& v) const {
...
std::vector<Edge*> MctsGraphOfConvexSets::Edges() {
...
std::vector<const Edge*> MctsGraphOfConvexSets::Edges() const {
...
bool MctsGraphOfConvexSets::IsValid(const Edge& e) const {
...
void MctsGraphOfConvexSets::ClearAllPhiConstraints() {
  for (const auto& e : edges_) {
    e.second->ClearPhiConstraints();
  }
}

std::string MctsGraphOfConvexSets::GetGraphvizString(
  ...
  graphviz << "digraph MctsGraphOfConvexSets {\n";
  graphviz << "labelloc=t;\n";
  for (const auto& [v_id, v] : vertices_) {
...
MctsGraphOfConvexSets::ConstructPreprocessingProgram(
...
std::set<EdgeId> MctsGraphOfConvexSets::PreprocessShortestPath(
    VertexId source_id, VertexId target_id,
    const MctsGraphOfConvexSetsOptions& options) const {
  if (!vertices_.contains(source_id)) {
    throw std::runtime_error(fmt::format(
        "Source vertex {} is not a vertex in this MctsGraphOfConvexSets.",
        source_id));
  }
  if (!vertices_.contains(target_id)) {
    throw std::runtime_error(fmt::format(
        "Target vertex {} is not a vertex in this MctsGraphOfConvexSets.",
        target_id));
  }
...
void MctsGraphOfConvexSets::AddPerspectiveCost(
  ...
  if (LinearCost* lc = dynamic_cast<LinearCost*>(cost)) {
    ...
    throw std::runtime_error(fmt::format(
        "MctsGraphOfConvexSets::Edge does not support this binding type: {}",
        binding.to_string()));
  }
}

void MctsGraphOfConvexSets::AddPerspectiveConstraint(
...
bool GcsIsThreadsafe(const MctsGraphOfConvexSets& gcs) {
...
MathematicalProgramResult MctsGraphOfConvexSets::SolveShortestPath(
    const Vertex& source, const Vertex& target,
    const MctsGraphOfConvexSetsOptions& specified_options) const {
...
std::vector<std::vector<const Edge*>> MctsGraphOfConvexSets::SamplePaths(
    const Vertex& source, const Vertex& target,
    const solvers::MathematicalProgramResult& result,
    const MctsGraphOfConvexSetsOptions& options) const {
  std::unordered_map<const Edge*, double> flows;
...
std::vector<std::vector<const Edge*>> MctsGraphOfConvexSets::SamplePaths(
  ...
  for (const auto& [edge, flow] : flows) {
    if (edges_.count(edge->id()) == 0) {
      throw std::invalid_argument(fmt::format(
          "Edge with id {} does not exist in this MctsGraphOfConvexSets.",
          edge->id()));
    }
  }
...
std::vector<const Edge*> MctsGraphOfConvexSets::GetSolutionPath(
    const Vertex& source, const Vertex& target,
...
std::vector<const Edge*> MctsGraphOfConvexSets::GetMctsSolutionPath(
  const Vertex& source, const Vertex& target,
  const solvers::MathematicalProgramResult& result, double tolerance) const {
  if (!result.is_success()) {
    throw std::runtime_error(
...
//     // implement MCTS tree search algorithm
//     // it might be easier to complete the algorithm with gcs::edge and gcs::vertex than string naming
...
std::unique_ptr<MathematicalProgram>
MctsGraphOfConvexSets::ConstructRestrictionProgram(
...
void MctsGraphOfConvexSets::MakeRestrictionResultLookLikeMixedInteger(
...
MathematicalProgramResult MctsGraphOfConvexSets::SolveConvexRestriction(
    const std::vector<const Edge*>& active_edges,
