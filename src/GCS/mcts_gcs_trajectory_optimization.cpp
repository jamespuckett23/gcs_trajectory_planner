#include "../../include/gcs_trajectory_planner/mcts_gcs_trajectory_optimization.h"
...
#include "../../include/gcs_trajectory_planner/mcts_graph_of_convex_sets.h"

namespace drake {
namespace planning {
namespace trajectory_optimization {

using Subgraph = MctsGcsTrajectoryOptimization::Subgraph;
using EdgesBetweenSubgraphs = MctsGcsTrajectoryOptimization::EdgesBetweenSubgraphs;
...
using geometry::optimization::MctsGraphOfConvexSets;
using geometry::optimization::MctsGraphOfConvexSetsOptions;
...
using Vertex = drake::geometry::optimization::MctsGraphOfConvexSets::Vertex;
using Edge = drake::geometry::optimization::MctsGraphOfConvexSets::Edge;
using VertexId = drake::geometry::optimization::MctsGraphOfConvexSets::VertexId;
using EdgeId = drake::geometry::optimization::MctsGraphOfConvexSets::EdgeId;
using Transcription = drake::geometry::optimization::MctsGraphOfConvexSets::Transcription;
...
Subgraph::Subgraph(
  ...
  MctsGcsTrajectoryOptimization* traj_opt)
  : regions_(regions),
    order_(order),
    h_min_(h_min),
    name_(std::move(name)),
...
std::vector<const MctsGraphOfConvexSets::Vertex*> Subgraph::Vertices() const {
std::vector<const MctsGraphOfConvexSets::Vertex*> vertices;
...
std::vector<const MctsGraphOfConvexSets::Edge*> Subgraph::Edges() const {
std::vector<const MctsGraphOfConvexSets::Edge*> edges;
...
Eigen::Map<const MatrixX<symbolic::Variable>> Subgraph::GetControlPoints(
  const geometry::optimization::MctsGraphOfConvexSets::Vertex& v) const {
...
EdgesBetweenSubgraphs::EdgesBetweenSubgraphs(
  ...
  const ConvexSet* subspace, MctsGcsTrajectoryOptimization* traj_opt,
...
std::vector<const MctsGraphOfConvexSets::Edge*> EdgesBetweenSubgraphs::Edges()
  const {
std::vector<const MctsGraphOfConvexSets::Edge*> edges;
edges.reserve(edges_.size());
for (const auto& e : edges_) {
...
EdgesBetweenSubgraphs::GetControlPointsU(
  const geometry::optimization::MctsGraphOfConvexSets::Edge& e) const {
...
MctsGcsTrajectoryOptimization::MctsGcsTrajectoryOptimization(
...
MctsGcsTrajectoryOptimization::~MctsGcsTrajectoryOptimization() = default;

Subgraph& MctsGcsTrajectoryOptimization::AddRegions(
    const ConvexSets& regions,
    const std::vector<std::pair<int, int>>& edges_between_regions, int order,
    double h_min, double h_max, std::string name,
    const std::vector<VectorXd>* edge_offsets) {
...
Subgraph& MctsGcsTrajectoryOptimization::AddRegions(const ConvexSets& regions,
  ...
  return MctsGcsTrajectoryOptimization::AddRegions(regions, edges_between_regions,
                                               order, h_min, h_max,
                                               std::move(name), &edge_offsets);
}

void MctsGcsTrajectoryOptimization::RemoveSubgraph(const Subgraph& subgraph) {
...
EdgesBetweenSubgraphs& MctsGcsTrajectoryOptimization::AddEdges(
    const Subgraph& from_subgraph, const Subgraph& to_subgraph,
...
void MctsGcsTrajectoryOptimization::AddTimeCost(double weight) {
...
void MctsGcsTrajectoryOptimization::AddPathLengthCost(
    const MatrixXd& weight_matrix) {
...
void MctsGcsTrajectoryOptimization::AddPathEnergyCost(
    const MatrixXd& weight_matrix) {
...
void MctsGcsTrajectoryOptimization::AddPathLengthCost(double weight) {
  const MatrixXd weight_matrix =
      weight * MatrixXd::Identity(num_positions(), num_positions());
  return MctsGcsTrajectoryOptimization::AddPathLengthCost(weight_matrix);
}

void MctsGcsTrajectoryOptimization::AddPathEnergyCost(double weight) {
  const MatrixXd weight_matrix =
      weight * 2 * MatrixXd::Identity(num_positions(), num_positions());
  return MctsGcsTrajectoryOptimization::AddPathEnergyCost(weight * weight_matrix);
}

void MctsGcsTrajectoryOptimization::AddVelocityBounds(
    const Eigen::Ref<const VectorXd>& lb,
    const Eigen::Ref<const VectorXd>& ub) {
  DRAKE_THROW_UNLESS(lb.size() == num_positions());
  DRAKE_THROW_UNLESS(ub.size() == num_positions());
...
void MctsGcsTrajectoryOptimization::AddNonlinearDerivativeBounds(
    const Eigen::Ref<const Eigen::VectorXd>& lb,
    const Eigen::Ref<const Eigen::VectorXd>& ub, int derivative_order) {
  DRAKE_THROW_UNLESS(lb.size() == num_positions());
  DRAKE_THROW_UNLESS(ub.size() == num_positions());
...
void MctsGcsTrajectoryOptimization::AddPathContinuityConstraints(
    int continuity_order) {
  if (continuity_order == 0) {
    throw std::runtime_error(
        "Path continuity is enforced by default. Choose a higher order.");
...
void MctsGcsTrajectoryOptimization::AddContinuityConstraints(int continuity_order) {
  if (continuity_order == 0) {
    throw std::runtime_error(
        "Path continuity is enforced by default. Choose a higher order.");
  }
...
MctsGcsTrajectoryOptimization::SolvePath(
  ...
  // add local function to facilitate MCTS search algorithm
  std::vector<const Edge*> path_edges =
      gcs_.GetMctsSolutionPath(*source_vertex, *target_vertex, result, kTolerance);
...
MctsGcsTrajectoryOptimization::SolveConvexRestriction(
    const std::vector<const Vertex*>& active_vertices,
    const MctsGraphOfConvexSetsOptions& options) {
...
MctsGcsTrajectoryOptimization::ReconstructTrajectoryFromSolutionPath(
  ...
  for (const Edge* e : edges) {
    ...
    if (num_control_points > 1 &&
      ...
      throw std::runtime_error(
          fmt::format("MctsGcsTrajectoryOptimization returned a trajectory segment "
  ...
  if (num_control_points > 1 && h < PiecewiseTrajectory<double>::kEpsilonTime) {
    throw std::runtime_error(fmt::format(
        "MctsGcsTrajectoryOptimization returned a trajectory segment with "
...
Edge* MctsGcsTrajectoryOptimization::AddEdge(Vertex* u, Vertex* v) {
  return gcs_.AddEdge(u, v, fmt::format("{} -> {}", u->name(), v->name()));
}

double MctsGcsTrajectoryOptimization::EstimateComplexity() const {
  double result = 0;
...
std::vector<Subgraph*> MctsGcsTrajectoryOptimization::GetSubgraphs() const {
...
MctsGcsTrajectoryOptimization::GetEdgesBetweenSubgraphs() const {
...
MctsGcsTrajectoryOptimization::NormalizeSegmentTimes(
...
trajectories::CompositeTrajectory<double>
MctsGcsTrajectoryOptimization::UnwrapToContinuousTrajectory(
